...
console-dev:
	_build/dev/rel/halin/bin/halin console

rel-prod:
	$(SED) -i 's/{halin, "halin-version"}/{halin, "$(VER)"}/g' ./rebar.config
	$(PWD)/script/rebar3 as prod release
	$(PWD)/script/rebar3 as prod tar
	$(SED) -i 's/{halin, "$(VER)"}/{halin, "halin-version"}/g' ./rebar.config
    #$(SCP) -P 8522 $(PWD)/_build/prod/rel/halin/halin-$(VER).tar.gz $(FS)
	@printf "\nApplication: %s\n" $(PWD)/_build/prod/rel/halin/halin-$(VER).tar.gz
...
rel-stage:
	...
	$(SED) -i 's/{halin, "$(VER)"}/{halin, "halin-version"}/g' ./rebar.config
    #$(SCP) -P 8522 $(PWD)/_build/stage/rel/halin/halin-$(VER).tar.gz $(FS)
	@printf "\nApplication: %s\n" $(PWD)/_build/stage/rel/halin/halin-$(VER).tar.gz
